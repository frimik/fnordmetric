class FnordMetric::Timeseries
  ...
  def trend(range = (ticks.first..ticks.last))
    ...
    rvals = @timeline.to_a.select{ |t,v| range.include?(t) }.sort{ |a,b| a.first <=> b.first }.map{ |t,v| value_at(t) }
  ...
  def to_json(&block)
    @timeline.to_a.sort{ |a,b| a[0] <=> b[0] }.map { |t,v| { :x => t, :y => block.call(*v), :v0 => v[0], :v1 => v[1] } }.to_json
  end
